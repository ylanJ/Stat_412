...
Sample3 <- Cluster3[sample(nrow(Cluster3), nrow(Cluster3)/nrow(Majority) * nrow(cs_train_min), replace = FALSE),]
Sample3
nrow(Sample3)
Cluster5 <- subset.data.frame(Majority_with_cluster, c(cluster == 5, cluster ==1, cluster == 2, cluster ==4))
Sample5 <- Cluster5[sample(nrow(Cluster5), nrow(Cluster5)/nrow(Majority) * nrow(cs_train_min), replace = FALSE),]
nrow(Sample5)
balanced_dataset.1 <- rbind(cs_train_min[-which(names(cs_train_min) == "X")], Sample3[-which(names(Sample3) == "cluster")], Sample5[-which(names(Sample5) == "cluster")])
...
### Model 1: GLM Model With Original Predictors - Training

### a) Residual Plot (Only training no testing)

```{r}
## Set model as final model
model.final <- model

plot(model.final, 1:2)
```
...
### c) Goodness of Fit using Hosmer-Lemeshow Test
...
### Model 1: GLM Model With Original Predictors - Testing

### d) Confusion Matrix/Accuracy - Testing
...
### PCA-Not sure how to predict with it (skip for now)
