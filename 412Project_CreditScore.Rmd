...
ggplot(data=Majority_with_cluster) + geom_point(aes(x=log(DebtRatio), 
                                       y= log(RevolvingUtilizationOfUnsecuredLines), colour=cluster))+theme_bw()

plot(age ~ log(MonthlyIncome), col= cluster, data = Majority_with_cluster)

plot(log(RevolvingUtilizationOfUnsecuredLines) ~ log(DebtRatio), col= cluster, data = Majority_with_cluster)
...
ggplot(data=Majority_with_cluster, aes(x=log(DebtRatio), 
                                       y= log(RevolvingUtilizationOfUnsecuredLines)))+geom_point(size=1) + facet_grid(~ cluster)
```
...
ridge_model5
Log.Likelihood <- ridge_model5$glmnet.fit$nulldev - ridge_model5$glmnet.fit$nulldev * (1- ridge_model5$glmnet.fit$dev.ratio[which(ridge_model5$glmnet.fit$lambda == ridge_model5$lambda.min)])
k <- ridge_model5$glmnet.fit$df[which(ridge_model5$glmnet.fit$lambda == ridge_model5$lambda.min)]
n <- ridge_model5$glmnet.fit$nobs
...
Log.Likelihood <- ridge_model_balanced$glmnet.fit$nulldev - ridge_model_balanced$glmnet.fit$nulldev * (1- ridge_model_balanced$glmnet.fit$dev.ratio[which(ridge_model_balanced$glmnet.fit$lambda == ridge_model_balanced$lambda.min)])
k <- ridge_model_balanced$glmnet.fit$df[which(ridge_model_balanced$glmnet.fit$lambda == ridge_model_balanced$lambda.min)]
...
lasso_model5
Log.Likelihood <- lasso_model5$glmnet.fit$nulldev - lasso_model5$glmnet.fit$nulldev * (1- lasso_model5$glmnet.fit$dev.ratio[which(lasso_model5$glmnet.fit$lambda == lasso_model5$lambda.min)])
k <- lasso_model5$glmnet.fit$df[which(lasso_model5$glmnet.fit$lambda == lasso_model5$lambda.min)]
n <- lasso_model5$glmnet.fit$nobs
AICc <- -Log.Likelihood + 2 * k + 2 * k * (k+1)/(n-k-1)
AICc
...
Log.Likelihood <- lasso_model_balanced$glmnet.fit$nulldev - lasso_model_balanced$glmnet.fit$nulldev * (1- lasso_model_balanced$glmnet.fit$dev.ratio[which(lasso_model_balanced$glmnet.fit$lambda == lasso_model_balanced$lambda.min)])
k <- lasso_model_balanced$glmnet.fit$df[which(lasso_model_balanced$glmnet.fit$lambda == lasso_model_balanced$lambda.min)]
n <- lasso_model_balanced$glmnet.fit$nobs
...
pred_compare = prediction(train_preds, train.y)
plot(performance(pred_compare, "acc"))
table(train.y, train_preds>0.66) # accurary on train
```
...
cs_train_m <- mutate(cs_train, predprob=predict(model.final, newx = as.matrix(train.x), type="response"))  # cal p^_i
gdf <- group_by(cs_train_m, ntile(linpred, 1000)) # group up the data by eta_x into 100 groups
hldf <- summarise(gdf, y=sum(SeriousDlqin2yrs==1), ppred=mean(predprob), count=n())
head(hldf)
...
pred_compare = prediction(train_preds, train.y)
plot(performance(pred_compare, "acc"))
table(train.y, train_preds>0.6) # accurary on train
```
